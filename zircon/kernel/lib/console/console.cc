...
static int cmd_clear(int argc, const cmd_args* argv, uint32_t flags);
static int cmd_smos(int argc, const cmd_args* argv, uint32_t flags);
...
STATIC_COMMAND_MASKED("clear", "clear the terminal screen", &cmd_clear, CMD_AVAIL_ALWAYS)
STATIC_COMMAND_MASKED("smos", "print the SMOS banner", &cmd_smos, CMD_AVAIL_ALWAYS)
...
#define SMOS_BANNER                                                 \
 ...
 " (_-</ -_) _| || | '_/ -_) '  \\/ _ \\ ' \\| |  _/ _ \\ '_|\n"    \
 " /__/\\___\\__|\\_,_|_| \\___|_|_|_\\___/_||_|_|\\__\\___/_|\n\n" \
...
// NOTE: Secure Monitor OS
//     ___ __ _  ___  ___
//    (_-</  ' \/ _ \(_-<
//   /___/_/_/_/\___/___/ (2026)
//
...
void kernel_shell_init() {
  if (!gBootOptions->shell_script.empty()) {
    dprintf(INFO, "entering zircon console\n");
