...
`release/api/client print bazel_sdk_info | fx jq  '.[] .location'`
...
```bash
$ export FUCHSIA_SDK_PATH="$(fx get-build-dir)/$(${FUCHSIA_DIR}/release/api/client print bazel_sdk_info | fx jq -r '.[] .location')"
$ export RULES_FUCHSIA_PATH="$(fx get-build-dir)/$(${FUCHSIA_DIR}/release/api/client print rules_fuchsia_info | fx jq -r '.[] .location')"
