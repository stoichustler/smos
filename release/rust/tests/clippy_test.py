...
FAKE_ROOT = HOST / "gen/release/rust/tests"
FAKE_OUT = FAKE_ROOT / "out/default"
TEST_DIR = Path("gen/release/rust/tests")
...
class TestClippy(unittest.TestCase):
    def test_expected(self) -> None:
        lints = read_lints(run_clippy("//release/rust/tests:a", "--raw"))
        codes = extract_codes(lints)
        self.assertEqual(codes, ["clippy::needless_return"])

    def test_unit_test(self) -> None:
        lints = read_lints(run_clippy("//release/rust/tests:a_test", "--raw"))
        codes = extract_codes(lints)
        self.assertEqual(
    ...
    def test_dedup_lints(self) -> None:
        lints = read_lints(
            run_clippy(
                "-f",
                FAKE_ROOT / "release/rust/tests/a/main.rs",
                FAKE_ROOT / "release/rust/tests/a/other.rs",
                "--raw",
            )
        )
        codes = extract_codes(lints)
    ...
    def test_depfiles(self) -> None:
        with open(FAKE_OUT / TEST_DIR / "a.aux.deps.deps") as f:
            self.assertEqual(
                f.read().splitlines(),
                ["--extern=b=obj/release/rust/tests/b.actual/libb.rlib"],
            )
        with open(FAKE_OUT / TEST_DIR / "b.aux.deps.deps") as f:
            self.assertEqual(
                f.read().splitlines(),
                ["--extern=c=obj/release/rust/tests/c.actual/libc.rlib"],
            )
        with open(FAKE_OUT / TEST_DIR / "a.aux.deps.transdeps") as f:
            self.assertIn(
                "-Ldependency=obj/release/rust/tests/b.actual",
                f.read().splitlines(),
            )

    def test_file_mapping(self) -> None:
        output = run_clippy(
            "--get-outputs", "-f", FAKE_ROOT / "release/rust/tests/b/lib.rs"
        )
        self.assertIn(str(TEST_DIR / "b.clippy"), set(output.splitlines()))

        output = run_clippy(
            "--get-outputs", "-f", FAKE_ROOT / "release/rust/tests/a/main.rs"
        )
        for label in {
    ...
    def test_dedup_files(self) -> None:
        ...
        self.assertEqual(
            sorted(outputs),
            [
                "gen/release/rust/tests/a.clippy",
                "gen/release/rust/tests/a_test.clippy",
            ],
        )
    ...
    def test_host_toolchain(self) -> None:
        lints = read_lints(
            run_clippy(
                f"//release/rust/tests:d(//release/toolchain:{HOST})", "--raw"
            )
        )
        codes = extract_codes(lints)
        self.assertEqual(codes, ["clippy::needless_return"])
        ...
        lints = read_lints(
            run_clippy("-f", FAKE_ROOT / "release/rust/tests/d/lib.rs", "--raw")
        )
        codes = extract_codes(lints)
        self.assertEqual(codes, ["clippy::needless_return"])

        with self.assertRaises(CalledProcessError):
            run_clippy("//release/rust/tests:d")
