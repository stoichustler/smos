...
source "$script_dir"/../release/rbe/common-setup.sh

project_root="$default_project_root"

readonly detail_diff="$project_root"/release/rbe/detail-diff.sh
...
function diff_file_relpath() {
  ...
  case "$filebase" in
    ...
    *.image_assembler_all_inputs) expect=skip ;;  # too many blob/hash differences
    ...
    java.log)
      case "$common_path" in
        gen/release/bazel/*/java.log) expect=ignore ;;  # log with timestamps
        *) expect=unknown ;;
      esac
      ;;
    ...
    lock)
      case "$common_path" in
        gen/release/bazel/*/lock) expect=ignore ;;  # contains PID
        *) expect=unknown ;;
      esac
      ;;
    ...
    command_port)
      case "$common_path" in
        gen/release/bazel/*/command_port) expect=ignore ;;  # randomly chosen port
        *) expect=unknown ;;
      esac
      ;;
    ...
    request_cookie)
      case "$common_path" in
        gen/release/bazel/*/request_cookie) expect=ignore ;;  # looks like hash
        *) expect=unknown ;;
      esac
      ;;
    ...
    response_cookie)
      case "$common_path" in
        gen/release/bazel/*/response_cookie) expect=ignore ;;  # looks like hash
        *) expect=unknown ;;
      esac
      ;;
    ...
    server_info.rawproto)
      case "$common_path" in
        gen/release/bazel/*/server_info.rawproto) expect=ignore ;;  # probably PID
        *) expect=unknown ;;
      esac
      ;;
    ...
    command.log)
      case "$common_path" in
        gen/release/bazel/*/command.log) expect=ignore ;;  # subject to build parallelism and completion ordering
        *) expect=unknown ;;
      esac
      ;;
    ...
    ffx.log)
      case "$common_path" in
        gen/release/bazel/*/cache/logs/ffx.log) expect=ignore ;;  # log with timestamps
        *) expect=unknown ;;
      esac
      ;;
