...
# Either update or check the content of //release/uci/jiri_generated/config.json
...
# Reads the repository revision without requiring nested Git metadata. Compact
# source distributions pin the revision in git-revision; full checkouts retain
# the Git lookup as the source of truth.
function get_icu_revision() {
  local directory="${1}"
  local revision
  if [[ -e "${directory}/.git" ]]; then
    revision="$(get_git_commit_id "${directory}")" || return 1
  elif [[ -f "${directory}/git-revision" ]]; then
    revision="$(tr -d '[:space:]' <"${directory}/git-revision")"
  else
    return 1
  fi
  if [[ ! "${revision}" =~ ^[0-9a-f]{40}$ ]]; then
    die "invalid ICU revision in ${directory}"
    return 1
  fi
  printf '%s\n' "${revision}"
}
...
default_commit_id="$(get_icu_revision "${icu_default_dir}")" ||
    die "commit id not found in $icu_default_dir"

latest_commit_id="$(get_icu_revision "${icu_latest_dir}")" ||
