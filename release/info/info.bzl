...
DEFAULT_PRODUCT_BUILD_INFO = {
    ...
    "version": "LABEL(@//release/info:version)",
    "jiri_snapshot": "LABEL(@//release/info:jiri_snapshot)",
    "latest_commit_date": "LABEL(@//release/info:latest_commit_date)",
}
...
gen_latest_date_and_timestamp = rule(
    ...
    attrs = {
        "_py_script": attr.label(
            ...
            default = "//release/info:gen_latest_commit_date.py",
        ),
        "_commit_hash_file": attr.label(
            allow_single_file = True,
            default = "//release/info:jiri_generated/integration_commit_hash.txt",
        ),
        "_commit_stamp_file": attr.label(
            allow_single_file = True,
            default = "//release/info:jiri_generated/integration_commit_stamp.txt",
        ),
    } | PY_TOOLCHAIN_DEPS,
)
...
# target as an alias to `//release/info:latest_commit_data`, otherwise write
...
def get_build_info_version(name):
    ...
    if build_info_version != "":
        ...
        native.alias(
            name = name,
            actual = "//release/info:latest_commit_date",
        )
