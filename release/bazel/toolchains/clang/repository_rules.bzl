...
def _generate_prebuilt_clang_toolchain_impl(repo_ctx):
    ...
    repo_ctx.symlink(
        workspace_dir + "/release/bazel/toolchains/clang/prebuilt_clang.BUILD.bazel",
        "BUILD.bazel",
    )

    repo_ctx.symlink(
        workspace_dir + "/release/bazel_sdk/bazel_rules_fuchsia/common",
        "common",
    )
...
def _generate_prebuilt_llvm_repository_impl(repo_ctx):
    ...
    script_path = str(repo_ctx.path(Label("@//:release/bazel/scripts/symlink-directory.py")))
    ...
    repo_ctx.symlink(
        workspace_dir + "/release/bazel/toolchains/clang/prebuilt_llvm.BUILD.bazel",
        "BUILD.bazel",
    )
