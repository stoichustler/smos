...
Then running `bazel build //userspace:foo` in the `/tmp/test_project` on Posix
...
For example, assume an `py_binary(//userspace:foo)` target that depends
...
...../bin/
  src/
    foo              # python middle-man for //userspace:foo py_binary()
