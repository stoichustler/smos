...
- The `BAZEL_TOPDIR` directory under the Ninja output directory is used
  ...
  value is `out/default/gen/release/bazel`.
...
- All four are generated by running `fx build bazel_workspace` (which ends
  up calling targets in `//release/bazel/BUILD.gn`). The workspace content has
  a specific layout that mirrors the Fuchsia source tree with a few exceptions:

  - A top-level `BUILD.bazel` symlink to `//release/bazel/toplevel.BUILD.bazel`
  ...
  - A top-level `WORKSPACE.bazel` generated from
    `//release/bazel/templates/template.WORKSPACE.bazel` in the source tree.
  ...
  - Symlinks to all top-level entries in the Fuchsia source tree, except for
    ...
    it will be visible and will define a Bazel package at `//userspace/foo:`.
...
- The GN `bazel_build_action()` template can be used to invoke a
  ...
  location. See `//release/bazel/bazel_build_action.gni` for details.

  The `//release/bazel/tests/build_action/` directory contains a working
...
- The Fuchsia platform build also relies on the new [Platforms][Platforms]
  ...
  build configuration. See `//release/bazel/platforms/BUILD.bazel` for more
  details.
...
directory through the `//userspace/lib:process_foo` GN target.

On the GN side:

```py
# From //userspace/lib/BUILD.gn, evaluated in the default toolchain context:
...
# as the @gn_targets//userspace/lib:foo filegroup()
bazel_input_file("foo.bazel_input") {
  generator = ":foo"
}

# A target that invokes a Bazel target to process the
# foo outputs. The result is copied to $BUILD_DIR/obj/userspace/lib/foo.final
...
```py
# From //userspace/lib/BUILD.bazel
genrule(
  name = "foo_processor",
  srcs = [ "@gn_targets//userspace/lib:foo" ],
  outs = [ "foo.processed_by_bazel" ],
  command = "process.sh $< $@",
)
```

Notice the `//userspace/lib:foo.bazel_input` GN target definition. This does not build
anything, but records information about the outputs of the *generator* target
`//userspace/lib:foo`.

The `//userspace/lib:process_foo` GN target depends on it, which will force the content
...
In this case, because `//userspace/lib:process_foo` depends on `//userspace/lib:foo.bazel_input`
in the GN graph, the Bazel `@gn_targets//userspace/lib:foo` filegroup will be defined,
grouping the outputs of the GN `//userspace/lib:foo` target, built or updated by Ninja
...
path such as `@gn_targets//obj/userspace/lib/foo.out`.
...
```py
# From //userspace/lib/BUILD.gn
action("foo") {
  ...
}

# This creates `@gn_targets//userspace/lib:foo`
bazel_input_file("foo_outputs") {
  generator = ":foo"
}

# This creates `@gn_targets//userspace/lib:foo_alt`, by overriding the
# name explicitly.
...
# From //userspace/lib/bar/BUILD.gn

# Only expose the (first) output of //userspace/lib:foo as @gn_targets//userspace/lib:foo
# The fact that this target is defined under //userspace/lib/bar/ does not matter.
bazel_input_file("foo.bazel_input") {
  generator = "//userspace/lib:foo"
...
Note that this generates the filegroup as `@gn_targets//userspace/lib:foo`, and not
as `@gn_targets//userspace/lib/bar:foo`.
...
`//release/bazel/bazel_build_action.gni` is used to create a GN action that
...
For a concrete example, see `//release/bazel/tests/build_action/` that
...
`//release/bazel/templates/template.bazelrc`,
...
You can invoke `release/bazel/scripts/test-all.py` to verify that everything
...
1) Invoke `release/bazel/scripts/prepare-fuchsia-checkout.py`, unless
...
Always try to add new tests when introducing new features under //release/bazel/,
...
to text automatically with `//release/bazel/scripts/parse-workspace-event-log.py`
