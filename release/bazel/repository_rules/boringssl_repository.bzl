...
def _boringssl_repository_impl(repo_ctx):
    ...
    # //release/bazel/workspace_utils.py.
    ...
    repo_ctx.execute(
        [
            repo_ctx.path(workspace_dir + "/release/bazel/scripts/hardlink-directory.py"),
            "--fuchsia-dir",
            workspace_dir,
            src_dir,
            dest_dir,
    ...
    repo_ctx.file("BUILD.bazel", content = repo_ctx.read(
        repo_ctx.path(workspace_dir + "/release/bazel/local_repositories/boringssl/BUILD.boringssl"),
    ), executable = False)
