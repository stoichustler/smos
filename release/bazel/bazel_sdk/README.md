...
in `$(fx get-build-dir)/gen/release/bazel/fuchsia_sdk` pointing to a directory
...
- Running the Bazel SDK test suite, which has its own workspace under
  `//release/bazel_sdk/tests`, which is critical to verify that all the
...
The GN target actually builds the `//release/bazel/bazel_sdk:final_fuchsia_in_tree_sdk`
...
GN Build Graph                                       |  Bazel Workspace
  ...
  //release/bazel:generate_fuchsia_sdk_repository      |                   |
          |                                          |                   v
          `------------------symlink---------------------> //release/bazel/bazel_sdk:final_fuchsia_in_tree_sdk
                                                     |
                                                     |
