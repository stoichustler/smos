...
class FilterBazelBinPathTest(unittest.TestCase):
    def test_good_paths(self):
        TEST_CASES = {
            ...
            "bazel-out/k8-fastbuild-ST-228372328ef8a/bin/build/tests/prog": "release/tests/prog",
...
class HostTestInfoTest(unittest.TestCase):
    TEST_INPUT = {
        "label": "@//userspace/foo:test_program",
    ...
    def test_construction_with_bad_label(self):
        ...
        test_input["label"] = "//userspace/foo:test_program"
        with self.assertRaises(AssertionError) as cm:
            export_host_tests.HostTestInfo(test_input, host_info)

        self.assertEqual(
            str(cm.exception),
            "Invalid Bazel target label does not begin with @: //userspace/foo:test_program",
        )
