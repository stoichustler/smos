...
When a [GN `fidl` rule](/release/fidl/fidl.gni) is defined for a FIDL library,
...
```gn
# //userspace/tictactoe/BUILD.gn

fidl("games.tictactoe") { ... }
```

The FIDL crate target is
`//userspace/tictactoe:games.tictactoe_rust`. To use the FIDL crate,
...
rustc_binary("tictactoe") {
  # ...
  deps = ["//userspace/tictactoe:games.tictactoe_rust"]
}
```
...
```bash
fx rustdoc //userspace/tictactoe:games.tictactoe_rust --open
```
