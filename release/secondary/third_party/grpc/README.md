...
This should be done in `//release/secondary/third_party/grpc`.
...
1. After `BUILD.gn` is updated in the previous section, merge changes from the target version and patch the updated `BUILD.gn` from `//release/secondary/third_party/grpc` to `//third_party/grpc`:
    ```
