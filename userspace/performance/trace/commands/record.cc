...
#include "userspace/performance/trace/commands/record.h"
...
#include "userspace/lib/fxl/strings/string_number_conversions.h"
#include "userspace/performance/trace/utils.h"
...
bool RecordCommand::Options::Setup(const fxl::CommandLine& command_line) {
  ...
  if (!binary) {
    FX_LOGS(ERROR) << "SMOS trace output is binary FXT; --binary=false is unsupported";
    return false;
  }
  if (binary) {
  ...
  if (compress) {
    FX_LOGS(ERROR) << "SMOS trace output does not support compressed FXT files";
    return false;
  }
  if (compress) {
    output_file_name += ".gz";
...
void RecordCommand::StartTimer() {
  ...
  out() << "Starting trace; will stop in "
        << static_cast<double>(options_.duration.to_nsecs()) / 1000000000.0
        << " seconds...\n";
}
