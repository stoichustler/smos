...
#include "userspace/performance/trace/app.h"

#include <lib/sys/cpp/component_context.h>

#include "userspace/performance/trace/commands/list_categories.h"
#include "userspace/performance/trace/commands/record.h"
#include "userspace/performance/trace/commands/time.h"
...
void App::Start(const fxl::CommandLine& command_line) {
  ...
  if (it == known_commands_.end()) {
    out() << "Unknown command '" << positional_args.front() << "' - aborting\n";
    PrintHelp();
    Done(EXIT_FAILURE);
    return;
  }
