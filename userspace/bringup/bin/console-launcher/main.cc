...
#include "userspace/bringup/bin/console-launcher/console_launcher.h"
#include "userspace/bringup/bin/console-launcher/console_launcher_config.h"
#include "userspace/lib/fxl/strings/split_string.h"
#include "userspace/lib/loader_service/loader_service.h"
#include "userspace/storage/lib/vfs/cpp/managed_vfs.h"
#include "userspace/storage/lib/vfs/cpp/pseudo_dir.h"
#include "userspace/storage/lib/vfs/cpp/remote_dir.h"
#include "userspace/storage/lib/vfs/cpp/vfs_types.h"
#include "userspace/storage/lib/vfs/cpp/vnode.h"
#include "userspace/sys/lib/stdout-to-debuglog/cpp/stdout-to-debuglog.h"
...
zx_status_t WaitForDriverBootup() {
  zx::result manager = component::Connect<fuchsia_driver_development::Manager>();
  if (manager.is_error()) {
    FX_PLOGS(ERROR, manager.status_value())
        << "failed to connect to "
        << fidl::DiscoverableProtocolName<fuchsia_driver_development::Manager>;
    return manager.status_value();
  }

  fidl::WireSyncClient client{std::move(manager.value())};
  fidl::WireResult result = client->WaitForBootup();
  if (!result.ok()) {
    FX_PLOGS(ERROR, result.status()) << "failed to wait for driver manager bootup";
...
constexpr std::string_view kSMOSBanner =
    "  ____ __  ___ ___\n"
    " (_-< '  \\/ _ (_-<\n"
    " /__/_|_|_\\___/__/ (2026)\n"
    "\n";

zx_status_t WriteSMOSBanner(fidl::ClientEnd<fuchsia_hardware_pty::Device>& stdio) {
  auto data = fidl::VectorView<uint8_t>::FromExternal(
      reinterpret_cast<uint8_t*>(const_cast<char*>(kSMOSBanner.data())), kSMOSBanner.size());
  fidl::WireResult result = fidl::WireCall(stdio)->Write(std::move(data));
  if (!result.ok()) {
    return result.status();
  }
  if (result.value()->actual_count != kSMOSBanner.size()) {
    return ZX_ERR_IO;
  }
  return ZX_OK;
}
...
[[noreturn]] void RunSerialConsole(const console_launcher::ConsoleLauncher& launcher,
  ...
  while (true) {
    ...
    fidl::ClientEnd<fuchsia_hardware_pty::Device> banner_stdio;
    if (!cmd.has_value()) {
      auto [banner_client, banner_server] =
          fidl::Endpoints<fuchsia_hardware_pty::Device>::Create();
      const fidl::Status banner_clone = fidl::WireCall(client)->Clone(
          fidl::ServerEnd<fuchsia_unknown::Cloneable>(banner_server.TakeChannel()));
      if (!banner_clone.ok()) {
        FX_PLOGS(ERROR, banner_clone.status()) << "failed to clone stdio for SMOS banner";
      } else {
        banner_stdio = std::move(banner_client);
      }
    }
    ...
    if (banner_stdio.is_valid()) {
      if (zx_status_t status = WriteSMOSBanner(banner_stdio); status != ZX_OK) {
        FX_PLOGS(ERROR, status) << "failed to write SMOS banner";
      }
    }
...
int main(int argv, char** argc) {
  ...
  // Delay all shell processes until driver startup has settled. The driver manager logs
  // "Bootup completed." before replying to this request.
  if (args.run_shell || !args.virtcon_disabled) {
    if (zx_status_t status = WaitForDriverBootup(); status != ZX_OK) {
      FX_PLOGS(FATAL, status) << "driver manager bootup wait failed";
    }
  }
