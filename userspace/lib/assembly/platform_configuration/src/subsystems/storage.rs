...
impl DefineSubsystemConfiguration<StorageConfig> for StorageSubsystemConfig {
    fn define_configuration(
        ...
        if matches!(context.feature_set_level, FeatureSupportLevel::Embeddable) {
                ...
                "Embeddable products must use filesystems.image_mode='no_image'"
            );
        } else if *context.feature_set_level == FeatureSupportLevel::Bootstrap {
            ensure!(
                matches!(
                    storage_config.filesystems.image_mode,
                    FilesystemImageMode::NoImage | FilesystemImageMode::Ramdisk
                ),
                "Bootstrap products must use filesystems.image_mode='no_image' or 'ramdisk'"
            );
        }
        ...
        if storage_config.include_paver.unwrap_or(true)
            && *context.feature_set_level != FeatureSupportLevel::Embeddable
