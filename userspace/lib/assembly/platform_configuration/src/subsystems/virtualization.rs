...
impl DefineSubsystemConfiguration<PlatformVirtualizationConfig> for VirtualizationSubsystem {
    fn define_configuration(
        ...
        if virtualization_config.bootstrap_enabled {
            ensure!(
                *context.feature_set_level == FeatureSupportLevel::Bootstrap,
                "Bootstrap virtualization requires the bootstrap feature set level"
            );
            builder.platform_bundle("virtualization_support");
        } else if virtualization_config.enabled {
            ensure!(
