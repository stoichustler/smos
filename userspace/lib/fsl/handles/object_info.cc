...
std::pair<zx_koid_t, zx_koid_t> GetKoids(zx_handle_t handle) {
  ...
  zx_status_t status =
      zx_object_get_info(handle, ZX_INFO_HANDLE_BASIC, &info, sizeof(info), nullptr, nullptr);
  return std::pair<zx_koid_t, zx_koid_t>(status == ZX_OK ? info.koid : ZX_KOID_INVALID,
...
zx::duration GetCurrentThreadTotalRuntime() {
  ...
  zx_status_t status = zx_object_get_info(zx_thread_self(), ZX_INFO_THREAD_STATS, &info,
...
size_t GetCurrentProcessMemoryMappedBytes() {
  ...
  zx_status_t status = zx_object_get_info(zx_process_self(), ZX_INFO_TASK_STATS, &info,
                                          sizeof(info), nullptr, nullptr);
