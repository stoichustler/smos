...
#include <userspace/lib/chunked-compression/streaming-chunked-compressor.h>
#include <zstd/zstd.h>
...
Status StreamingChunkedCompressor::Init(size_t stream_len, void* output, size_t output_len) {
  ...
  if (ZSTD_isError(r)) {
    FX_LOG_KV(ERROR, "Failed to init stream");
...
Status StreamingChunkedCompressor::AppendToFrame(const void* data, size_t len) {
  ...
  if (ZSTD_isError(r)) {
    FX_LOG_KV(ERROR, "ZSTD_compressStream failed", FX_KV("status", r),
              FX_KV("status_str", ZSTD_getErrorName(r)));
    return kStatusErrInternal;
  } else if (in_buf.pos < in_buf.size) {
    FX_LOG_KV(ERROR, "Partial read");
    return kStatusErrInternal;
  }
  ...
  if (will_finish_frame) {
    ...
    if (ZSTD_isError(r)) {
      FX_LOG_KV(ERROR, "ZSTD_endStream failed", FX_KV("status", r),
                FX_KV("status_str", ZSTD_getErrorName(r)));
      return kStatusErrInternal;
    }
  }
  ...
  if (will_finish_frame) {
    ...
    if (status != kStatusOk) {
      FX_LOG_KV(ERROR, "Failed to finalize frame");
      return status;
    }
    ...
    if (input_offset_ < input_len_) {
      status = StartFrame();
      if (status != kStatusOk) {
        FX_LOG_KV(ERROR, "Failed to start next frame");
      }
    }
  } else {
