...
#include "userspace/storage/fvm/fvm_sparse.h"
#include "userspace/storage/fvm/metadata.h"
#include "userspace/storage/volume_image/address_descriptor.h"
#include "userspace/storage/volume_image/fvm/fvm_sparse_image.h"
#include "userspace/storage/volume_image/utils/lz4_decompressor.h"
 ...
 public:
  DecompressionHelper(Reader& base_reader, uint64_t start_offset)
    ...
    ZX_ASSERT(decompressor_
                  .Prepare([this](std::span<const uint8_t> decompressed_data) {
      ...
      while (decompressed_buffer_.size() < std::min(kBufferSize, buffer.size() - done)) {
        ...
        if (compressed_buffer_.size() < kBufferSize && compressed_offset_ < base_reader_.length()) {
          ...
          auto result = base_reader_.Read(
              compressed_offset_, std::span<uint8_t>(&compressed_buffer_[current_size], len));
          if (result.is_error()) {
        ...
        if (!compressed_buffer_.empty()) {
          ...
          auto result = decompressor_.Decompress(std::span<uint8_t>(compressed_buffer_));
          if (result.is_error()) {
            return result.take_error_result();
          }
...
class SparseImageReader : public Reader {
 public:
  ...
  fpromise::result<void, std::string> Read(uint64_t offset,
                                           std::span<uint8_t> buffer) const override {
