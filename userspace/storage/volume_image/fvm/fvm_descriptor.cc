...
std::string ToSizeString(uint64_t bytes) {
  constexpr int kByteToMebibyte = 1 << 20;
  std::string size_str = std::to_string(static_cast<double>(bytes) / kByteToMebibyte);
  return size_str.append(" [MiB]");
}
...
fpromise::result<void, std::string> FvmDescriptor::WriteBlockImage(Writer& writer) const {
  ...
  auto metadata_view = std::span<const uint8_t>(
  ...
  for (const auto& partition : partitions()) {
    for (const auto& mapping : partition.address().mappings) {
      ...
      for (uint64_t slice = 0; slice < slice_count; ++slice) {
        auto slice_data_view = std::span<uint8_t>(slice_buffer);
