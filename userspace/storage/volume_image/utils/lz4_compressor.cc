...
#include "userspace/storage/volume_image/utils/lz4_result.h"
...
fpromise::result<void, std::string> Lz4Compressor::Prepare(Handler handler) {
  ...
  return handler_(std::span(compression_buffer_.data(), result.byte_count()));
}
...
fpromise::result<void, std::string> Lz4Compressor::Finalize() {
  ...
  auto handler_result = handler_(std::span(compression_buffer_.data(), result.byte_count()));
