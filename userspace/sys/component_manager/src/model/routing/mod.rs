...
/// Logs a concise failure to route a capability.
pub async fn report_routing_failure(
    ...
    match availability {
        Availability::Required => {
            ...
            target.log(log::Level::Warn, format!("{capability_requested} unavailable"), &[]).await;
        }
        Availability::Optional | Availability::SameAsTarget | Availability::Transitional => {
            ...
            target.log(
                log::Level::Info,
                format!("{availability} {capability_requested} unavailable"),
                &[],
            ).await;
        }
    }
