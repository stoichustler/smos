...
#include "userspace/devices/bus/drivers/pci/bus.h"
...
#include "userspace/devices/bus/drivers/pci/bridge.h"
...
zx_status_t Bus::Initialize() {
  ...
  if (zx::result<std::optional<PciFidl::BoardConfiguration>> result =
          ddk::GetMetadataIfExists<PciFidl::BoardConfiguration>(parent_);
      result.is_ok() && result.value().has_value()) {
    board_config_ = std::move(result.value().value());
  }
