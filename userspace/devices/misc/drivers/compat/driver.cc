...
#include "userspace/devices/misc/drivers/compat/driver.h"
...
#include "userspace/devices/misc/drivers/compat/compat_driver_server.h"
#include "userspace/lib/driver_symbols/symbols.h"
  ...
  auto start_driver =
      Driver::ConnectToParentDevices()
          ...
          .then([this](result<void, zx_status_t>& result) -> fpromise::result<void, zx_status_t> {
            if (result.is_error()) {
              if (node_name().value_or("") == "dev") {
                logger_->log(fdf::INFO, "Getting DeviceInfo failed with: {}",
