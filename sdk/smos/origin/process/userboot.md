...
the [kernel "command line"] giving boot options, and RAM
...
system calls through the [vDSO](../kernel/vdso.md) like any other process would, and
is subject to the full [vDSO enforcement](../kernel/vdso.md#Enforcement) regime.
What's special about `userboot` is the way it gets loaded.

`userboot` is built as an ELF dynamic shared object, using the
same [RODSO layout](../kernel/vdso.md#read-only-dynamic-shared-object-layout) as
...
In normal [program loading], a [*bootstrap message*] is
sent to each new process.  The process's first thread receives
a [channel] handle in a register.  It can then read
...
the [`PA_VMO_VDSO` handle].

## userboot finds system calls in the vDSO

The [standard convention](../kernel/vdso.md#process_start_argument) for informing
...
a [VMO] containing the ZBI from the boot loader.  `userboot` reads the
ZBI headers from this VMO looking for the first item with type
`ZBI_TYPE_STORAGE_BOOTFS`.  That contains the [BOOTFS](#BOOTFS) image.
The item's ZBI header indicates if it's compressed, which it usually is.
`userboot` maps in this portion of the VMO.  `userboot` contains zstd
format support code, which it uses to decompress the item into a fresh VMO.
...
standard [`processargs`] messages.  It passes on all the important handles
it received from the kernel (replacing specific handles such as the
process-self and thread-self handles with those for the new process rather
than for `userboot` itself).
...
If [the `userboot.shutdown` option was given on the kernel command line],
