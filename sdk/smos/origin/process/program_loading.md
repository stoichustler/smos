...
[`Virtual Memory Objects`], [`processes`], [`Virtual Memory Address Regions`],
and [`threads`].
...
The standard Zircon userspace environment uses the
...
use [system calls] only through the Zircon [vDSO](../kernel/vdso.md), which is
...
program loading is based on [VMOs] and on IPC protocols used through [channels].
 ...
 * A list of initial [handles], each with
...
### An ELF `ET_DYN` file with no `PT_INTERP`
  ...
  * The system chooses a random base address for the first `PT_LOAD` segment
    ...
    This is done by creating a [VMAR] covering
...
the [vDSO](../kernel/vdso.md). The process has only this one handle and so it can
...
See the [header file](../../../../../zircon/system/public/zircon/processargs.h)
for full details of the message format. It's anticipated that this ad hoc
protocol will be replaced with a formal IDL-based protocol eventually,
but the format will be kept simple enough to be decoded by simple
hand-written code.

A bootstrap message conveys:

 * a list of initial [handles]
 ...
 * essential handles for the process to make [system calls]:
   [process], [VMAR], [thread], [job]
 * [channel] to the [loader service](#the-loader-service)
 * [vDSO](../kernel/vdso.md) [VMO]
...
Instead, program loading is based only on [VMOs] and
a simple [channel]-based protocol.
...
implementation can use the [vDSO](../kernel/vdso.md) [system calls].
