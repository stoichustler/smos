...
the `fx set` command or the `//release/rust:cargo_toml_gen` target to `$OUT_DIR/args.gn`'s
...
need to use the `//release/rust:cargo_toml_gen` build target instead. This target will only
