...
`//userspace/sys/lib/cm_rust:cm_rust_test($host_toolchain)` target.
...
```sh
$ fx set PRODUCT.BOARD --with //userspace/sys/lib/cm_rust:tests --cargo-toml-gen
```
...
```sh
$ fx gen-cargo '//userspace/sys/lib/cm_rust:cm_rust_test_executable(//release/toolchain:host_x64)'
```
...
without using GN variables (`//release/toolchain:host_x64`).
